...
(function($) {
		...
		'use strict';
			...
			var BasicFunction = function(){
				...
				var setAllCourseCarousel = function() {
					if(!checkSelectorExistence('.courses-carousel-all-courses')){return;}
					jQuery('.courses-carousel-all-courses').owlCarousel({
						loop:false,
						autoplay:false,
						margin:0,
						nav:true,
		...
		var ThemeBuilder = function(){
				...
				function setDefaultMasonry(){
                    var self = $("#masonry");
                    var filter = "happening";
                      self.masonryFilter({
                         filter: function () {
                            if (!filter) return true;
                            return $('.happening').hasClass(filter);
                         }
                      });

                      setTimeout(function() {$('#happening').click();},50)
                }
